...
class CreateAccountByVerifiedMobileRequest(BaseModel):
    ...
    day_of_birth: Optional[Annotated[str, Field(strict=True)]] = Field(
        default=None, alias="dayOfBirth"
    )
    ...
    month_of_birth: Optional[Annotated[str, Field(strict=True)]] = Field(
        default=None, alias="monthOfBirth"
    )
    name: StrictStr
    ...
    def day_of_birth_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^(0?[1-9]|[12][0-9]|3[01])$", value):
            raise ValueError(
                r"must validate the regular expression /^(0?[1-9]|[12][0-9]|3[01])$/"
            )
        return value
