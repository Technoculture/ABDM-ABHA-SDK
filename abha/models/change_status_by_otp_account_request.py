...
class ChangeStatusByOtpAccountRequest(BaseModel):
    ...
    auth_method: StrictStr = Field(
        description="Based on authMethods", alias="authMethod"
    )
    otp: Optional[StrictStr] = Field(default=None, description="Encrypted Mobile OTP.")
    password: Optional[StrictStr] = None
    reactivation_date: Optional[StrictStr] = Field(
        default=None, alias="reactivationDate"
    )
    reasons: Optional[List[StrictStr]] = None
    txn_id: Optional[StrictStr] = Field(
        default=None, description="Based on UUID", alias="txnId"
    )
    ...
    def auth_method_validate_enum(cls, value):
        ...
        if value not in set(
            ...
            raise ValueError(
                "must be one of enum values ('AADHAAR_OTP', 'MOBILE_OTP', 'PASSWORD', 'DEMOGRAPHICS', 'AADHAAR_BIO')"
            )
        return value
