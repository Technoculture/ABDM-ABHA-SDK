...
class CreateAccountByVerifiedMobileRequest(BaseModel):
    ...
    day_of_birth: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, alias="dayOfBirth")
    ...
    month_of_birth: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, alias="monthOfBirth")
    name: StrictStr
    ...
    token: StrictStr = Field(description="JWT Bearer Token")
    ...
    year_of_birth: Annotated[str, Field(strict=True)] = Field(alias="yearOfBirth")
    __properties: ClassVar[List[str]] = ["address", "dayOfBirth", "districtCode", "email", "firstName", "gender", "healthId", "lastName", "middleName", "monthOfBirth", "name", "password", "pincode", "profilePhoto", "stateCode", "subdistrictCode", "token", "townCode", "txnId", "villageCode", "wardCode", "yearOfBirth"]

    @field_validator('day_of_birth')
    def day_of_birth_validate_regular_expression(cls, value):
    ...
    def gender_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^(M|F|O|U)$", value):
            raise ValueError(r"must validate the regular expression /^(M|F|O|U)$/")
        return value

    @field_validator('gender')
    def gender_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['M', 'F', 'O', 'U']):
            raise ValueError("must be one of enum values ('M', 'F', 'O', 'U')")
        return value

    @field_validator('month_of_birth')
    def month_of_birth_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^([1-9]|1[012])$", value):
            raise ValueError(r"must validate the regular expression /^([1-9]|1[012])$/")
        return value

    @field_validator('year_of_birth')
    def year_of_birth_validate_regular_expression(cls, value):
        """Validates the regular expression"""
