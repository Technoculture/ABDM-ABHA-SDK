...
with abha.ApiClient(configuration) as api_client:
    ...
    auth_request = (
        abha.AuthWithMobileTxnAndUserData()
    ...
    try:
        ...
        api_response = api_instance.auth_with_mobile_token_using_post(
            auth_request, accept_language=accept_language
        )
        print("The response of AuthenticationApi->auth_with_mobile_token_using_post:\n")
        pprint(api_response)
    except Exception as e:
        print(
            "Exception when calling AuthenticationApi->auth_with_mobile_token_using_post: %s\n"
            % e
        )
