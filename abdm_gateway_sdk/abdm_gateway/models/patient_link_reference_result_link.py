...
class PatientLinkReferenceResultLink(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["referenceNumber", "authenticationType", "meta"]

    @field_validator('authentication_type')
    def authentication_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DIRECT', 'MEDIATED']):
            raise ValueError("must be one of enum values ('DIRECT', 'MEDIATED')")
        return value
