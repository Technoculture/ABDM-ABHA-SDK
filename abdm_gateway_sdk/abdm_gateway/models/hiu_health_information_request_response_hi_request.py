...
class HIUHealthInformationRequestResponseHiRequest(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["transactionId", "sessionStatus"]

    @field_validator('session_status')
    def session_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['REQUESTED', 'ACKNOWLEDGED']):
            raise ValueError("must be one of enum values ('REQUESTED', 'ACKNOWLEDGED')")
        return value
